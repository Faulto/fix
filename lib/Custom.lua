-- {"ver":"3.1.6","author":"TechnoJo4","dep":["url","dkjson"]}

local encode = Require("url").encode

local json = require("dkjson").aes_decrypt
...
function defaults:getPassage(url)
    ...
    local encryptedContent = htmlElement:select("#encrypted-content")
    if encryptedContent then
        local key = "Kj9#mPx$2vLq&8wR!tY5nZ@bQ4eH7uSd"
        local iv = "rT6$kP!xN9&mW3vJ"
		local decryptedText = json(tostring(encryptedContent), key, iv)
        htmlElement:html(decryptedText)
    end
